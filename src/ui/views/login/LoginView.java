...
/**
 * Vista de Login — Polla Mundialista 2026
 *
 * Estructura fiel al diseño:
 *  - Fondo blanco con grilla sutil
 *  - Card centrada 448×auto, border-radius 12px, sombra suave
 *  - Cabecera: círculo 64px (verde primario) + título 32px bold + subtítulo
 *  - Campos: label SemiBold + input 46px altura con ícono izquierdo
 *  - Botón verde ancho completo 48px altura
 *  - Link azul centrado
 */
public class LoginView extends JPanel {

    private static final int CARD_W    = 448;
    private static final int PAD       = 40;    // padding interno card
    private static final int INNER_W   = CARD_W - PAD * 2; // 368px
    ...
    private JPanel buildCard() {
        // Panel que pinta su propio fondo redondeado
        JPanel card = new JPanel(null) {  // null layout: posicionamos todo a mano
            @Override
            protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
        ...
        // ── Construir filas y calcular altura total ──
        int y = PAD;

        // Fila 0: Círculo ícono (64×64 centrado)
        JPanel circle = buildCircle(AppColors.PRIMARY_CONTAINER, "\u2192", AppColors.ON_PRIMARY_CONTAINER);
        ...
        // Fila 9: Botón Ingresar
        btnIngresar = buildButton("Ingresar  \u2192");
        ...
        // Fila 10: Link registro
        JPanel linkPanel = buildLink("\u00bfNo tienes cuenta? Reg\u00edstrate aqu\u00ed",
                e -> mainFrame.showView(MainFrame.VIEW_REGISTER));
    ...
    private JPanel buildInput(JTextField field, String placeholder,
        ...
        // Panel que pinta el borde redondeado + fondo blanco
        JPanel wrapper = new JPanel(null) {
            @Override
            protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                // Fondo blanco recortado a bordes redondeados
                g2.setColor(Color.WHITE);
                g2.fill(new RoundRectangle2D.Float(0, 0, getWidth(), getHeight(), RADIUS, RADIUS));
                // Borde: verde si focused, gris si no
        ...
        // Ícono (posición absoluta: left=12, verticalmente centrado)
        ...
        // Campo de texto (posición absoluta: left=40, ocupa el resto)
